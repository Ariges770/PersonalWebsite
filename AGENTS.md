...
- **`npm run dev` breaks through the Coder preview proxy (only works when accessed directly on `localhost`).** The user views the site via the Coder proxy URL (e.g. `https://dev-server--fix-reading--ariges770.coder...`), which proxies port 3000. In dev, Vite serves CSS files (KaTeX, Vue SFC styles, global CSS) as **JS modules** for HMR; the Coder proxy returns those as `text/css` and adds a long `max-age`, so the module graph fails (`Failed to load module script: MIME type "text/css"`, `Failed to fetch dynamically imported module: .../entry.js`) and **no client-side JS runs** — symptoms like the reading TOC highlight staying frozen on Q1. The **production build works** through the proxy (CSS is extracted to real `.css` files). So: keep port 3000 serving the production build (`node .output/server/index.mjs`) for preview, and run `npm run dev -- --port 3001` for HMR development accessed directly. Watch out for a Coder dev-server watchdog that restarts `npm run dev` on port 3000 — it will silently re-break the preview; kill it and re-serve the production build on 3000 if the preview loses JS.
